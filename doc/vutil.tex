...
The \cmdlink{var} TclOO class creates linked object variables that are tied to local variables, using \cmdlink{link} and \cmdlink{tie}.

\begin{syntax}
\command{var} new \$refName <{}<"="> \$value | "<-" \$varObj> \\
...
The method ``\texttt{\methodlink[0]{var}{=}}'' assigns the value of the object variable.
\begin{syntax}
\method{var}{=} \$value
\end{syntax}
\begin{args}
...
\end{args}
The method ``\texttt{\methodlink[0]{var}{<-}}'' assigns the value of the object directly from another object variable of the same class.
\begin{syntax}
...
\end{args}
The method ``\texttt{\methodlink[0]{var}{-{}->}}'' copies the object variable.
\begin{syntax}
\method{var}{-{}->} \$refName <\$arg ...>
\end{syntax}
\begin{args}
\$refName & Name of reference variable to tie to object. \\
\$arg ... & Additional arguments to pass to Tcl \textit{oo::copy} command.
\end{args}
The method \methodlink[0]{var}{info} accesses object variable metadata. Fields ``exists'' and ``type'' always exist, and ``value'' is populated when the variable is initialized.
\begin{syntax}
...
The \cmdlink{var} TclOO class acts as a superclass for a pure-Tcl type system, managed through the metaclass \cmdlink{type}.
\begin{syntax}
...
Then, using the types defined by \cmdlink{type}, the command \cmdlink{new} creates a new variable object of specified type.

\begin{syntax}
\command{new} \$type \$refName <{}<"="> \$value | "<-" \$varObj>
\end{syntax}
\begin{args}
\$type & Name of type. \\
\$refName & Name of reference variable to tie to object. \\
...
\begin{syntax}
\subcommand{new}{var} \$refName <{}<"="> \$value | "<-" \$varObj>
\end{syntax}
\begin{args}
\$refName & Name of reference variable to tie to object. \\
\$value & Value to set object variable to (using the optional ``\texttt{=}'' keyword). \\
...
\begin{syntax}
\subcommand{new}{string} \$refName <{}<"="> \$value | "<-" \$stringObj>
\end{syntax}
\begin{args}
\$refName & Name of reference variable to tie to object. \\
\$value & Value to set object variable to (using the optional ``\texttt{=}'' keyword). \\
...
\begin{syntax}
\subcommand{new}{bool} \$refName <{}<"="> \$expr | "<-" \$boolObj>
\end{syntax}
\begin{args}
\$refName & Name of reference variable to tie to object. \\
...
\begin{syntax}
\subcommand{new}{int} \$refName <{}<"="> \$expr | "<-" \$intObj>
\end{syntax}
\begin{args}
\$refName & Name of reference variable to tie to object. \\
\$expr & Tcl math expression to evaluate (using the optional ``\texttt{=}'' keyword). \\
...
The type \subcmdlink[0]{new}{float} passes input through the Tcl \textit{expr} command and the \texttt{::tcl::mathfunc::double} command, ensuring that the value is a double-precision floating-point number.
\begin{syntax}
\subcommand{new}{float} \$refName <{}<"="> \$expr | "<-" \$floatObj>
\end{syntax}
\begin{args}
\$refName & Name of reference variable to tie to object. \\
\$expr & Tcl math expression to evaluate (using the optional ``\texttt{=}'' keyword). \\
...
\begin{syntax}
\subcommand{new}{list} \$refName <{}<"="> \$value | "<-" \$listObj>
\end{syntax}
\begin{args}
\$refName & Name of reference variable to tie to object. \\
\$value & Value to set object variable to (using the optional ``\texttt{=}'' keyword). \\
...
\end{args}
The method \cmdlink{\$listObj length} returns the list length. This is the same as \texttt{\$listObj info length}.
\begin{syntax}
\command{\$listObj length}
\end{syntax}
The method \cmdlink{\$listObj @} acts as both list indexing and list setting, depending on whether the ``='' keyword is used.
If indexing, it returns the value at the specified index. If setting, it sets the value and returns the object name.
\begin{syntax}
\command{\$listObj @} \$i ... <''='' \$value>
\end{syntax}
\begin{args}
\$i ... & List indices. \\
\$value & Value to set. 
...
\begin{syntax}
\subcommand{new}{dict} \$refName <{}<"="> \$value | "<-" \$tObj>
\end{syntax}
\begin{args}
\$refName & Name of reference variable to tie to object. \\
\$value & Value to set object variable to (using the optional ``\texttt{=}'' keyword). \\
...
\end{args}
The method \cmdlink{\$dictObj size} returns the dictionary size. This is the same as \texttt{\$dictObj info size}.
\begin{syntax}
\command{\$dictObj size}
\end{syntax}
