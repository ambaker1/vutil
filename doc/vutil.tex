...
The operator ``\textbf{\texttt{\methodlink[0]{var}{\&}}}'' simply returns the name of the object.
\begin{syntax}
\method{var}{\&}
\end{syntax}
...
To remove a type, use the method \subcmdlink[0]{type}{remove}. 
...
The method \subcmdlink[0]{type}{isa} checks if an object is of a specific type or of one of its subtypes.
If the type or object does not exist, this command will return an error.

\begin{syntax}
\subcommand{type}{isa} \$type \$objName
\end{syntax}
\begin{args}
\$type & Name of type. \\
...
\$op & Assignment operator: \\
 & ``\texttt{+=}'' Increments the variable by the value of \texttt{\$value}. \\
 & ``\texttt{-=}'' Decrements the variable by the value of \texttt{\$value}. \\
...
2
world
exists 1 length 3 type list value {hey there world 2}
\end{lstlisting}
\end{example}
\clearpage
