...
This shared reference, regardless of scope, can be accessed with the global command ``\textbf{\texttt{\cmdlink{\$\&}}}''.
\begin{syntax}
\command{\$\&} \$arg ...
\end{syntax}
\begin{args}
...
\clearpage
\subsection{Self-Referencing Math Operators}
The operators ``\texttt{\methodlink[0]{var}{+=}}'', ``\texttt{\methodlink[0]{var}{-=}}'', ``\texttt{\methodlink[0]{var}{*=}}'', and ``\texttt{\methodlink[0]{var}{/=}}'' add, subtract, multiply, and divide the existing value of the object variable with the value given. 
Like the ``\texttt{\methodlink[0]{var}{=}}'' operator, this also returns the name of the object.
\begin{syntax}
\method{var}{+=} \$expr \\
\method{var}{-=} \$expr \\
\method{var}{*=} \$expr \\
...
The operator ``\texttt{\methodlink[0]{var}{.}}'' with no arguments simply returns the value of the object. If arguments are provided, it returns the value after passing it through a ``\texttt{::tcl::mathop}'' operator.
\begin{syntax}
\method{var}{.} <\$op <\$arg ...>{}>
\end{syntax}
\begin{args}
\$op & Valid ``\texttt{::tcl::mathop}'' operator. \\
\$arg ... & Input arguments to mathop command.
\end{args}

\begin{example}{Sum of values}
\begin{lstlisting}
puts [[var new x 0] . + 1 2 3 4 5]
\end{lstlisting}
\tcblower
\begin{lstlisting}
15
...
\$value & Integer value.
\end{args}
In addition to the standard math operators, this type also has two short-hand increment/decrement operators, ``\texttt{++}'' and ``\texttt{-{}-}'', which simply increment or decrement the integer object by 1.
\begin{syntax}
\$intObj ++ 
\end{syntax}
\begin{syntax}
...
The type ``\subcmdlink[0]{new}{dict}'' validates that the input is a Tcl dictionary, and provides methods for getting/setting/unsetting dictionary values, checking if values exist, and getting the dictionary size. Additionally, it adds the field ``size'' to the variable info. Self-referencing math operators are disabled for this type.
\begin{syntax}
