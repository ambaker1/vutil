...
(::+|\w+)+(\(\w+\))?
\end{lstlisting}
The one exception to this rule is the shared global reference ``\textbf{\texttt{\&}}'', stored in the variable \texttt{::vutil::\$\&}.
...
For example, ``\texttt{\$@ref}'' is converted to ``\texttt{\$\{::vutil::\$@(ref)\}}''.
...
\subsection{Self-Referencing Assignment Operators}
...
Similarly, the operators ``\texttt{\methodlink[0]{var}{:=}}'' and ``\texttt{\methodlink[0]{var}{::=}}'', allow for object self-reference using the command ``\cmdlink{\$.}''.
\begin{syntax}
\command{\$.} <\$arg ...>
\end{syntax}
\begin{args}
...
If setting with the ``\texttt{=}'' operator, it simply performs an lset command and returns the object name. 
If setting with the other assignment operators, it creates a temporary object variable (class ``\texttt{::vutil::var}'') with the current value, and evaluates the assignment operator, returning the list object name. 
\begin{syntax}
\$listObj @ \$i ... <''='' \$value | ''.='' \$oper | '':='' \$expr | ''::='' \$body>
\end{syntax}
\begin{args}
\$i ... & List indices. \\
\$value & Value to set. \\
...
This package only provides a new, slightly modified way to access the Tcl dictionary data structure, which is fully explained and documented here: \url{https://www.tcl.tk/man/tcl/TclCmd/dict.html}.
\begin{syntax}
...
\$string ... & Text to append to dictionary.
\end{args}
