...
The command \cmdlink{local} has the same syntax as the Tcl command \textit{global}, but instead of making global variables available in a procedure, it simply calls the Tcl command \textit{variable} multiple times to create the local namespace variables.
\begin{syntax}
\command{local} <\$varName ...>
\end{syntax} 
\begin{args}
...
The \cmdlink{pvar} command is a short-hand function for printing the name and value of Tcl variables, including arrays (like \textit{parray})
\begin{syntax}
\command{pvar} \$varName ...
\end{syntax}
\begin{args}
...
The \cmdlink{obj} TclOO class creates container objects tied to variables, making garbage collection easy. 
It is a simple 


\clearpage
\section{Interactive Workspace Viewer} 
The command \cmdlink{viewVars} creates an interactive table of variables and their values. 
Variables cannot be edited in the variable viewer window, but their values can be selected and copied. 
This command in particular requires the packages ``Tk'' and ``Tktable''.
\begin{syntax}
\command{viewVars} <\$varList>
\end{syntax}
\begin{args}
\$varList & Variables to view. Default uses the result of the Tcl command \textit{info vars}.
\end{args}
