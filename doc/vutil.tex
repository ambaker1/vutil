...
The \cmdlink{vputs} command is a short-hand function for printing the name and values of Tcl variables, in the same style as the Tcl \textit{parray} command.
\begin{syntax}
\command{vputs} \$name1 \$name2 …
\end{syntax}
\begin{args}
...
In addition to tying the object to a reference variable in the superclass constructor, the \cmdlink{::vutil::gcoo} superclass also provides methods for copying the object to a new reference variable: ``\texttt{\methodlink[0]{gcoo}{-{}->}}'' and ``\textbf{\texttt{\methodlink[0]{gcoo}{\&}}}''.
Note that the method ``\textbf{\texttt{\methodlink[0]{gcoo}{\&}}}'' is simply shorthand for copying to the global variable ``\textbf{\texttt{\&}}''.
\begin{syntax}
\method{gcoo}{-{}->} \$refName
\end{syntax}
\begin{syntax}
...
So, in addition to the superclass methods ``\texttt{\methodlink[0]{gcoo}{-{}->}}'' and ``\textbf{\texttt{\methodlink[0]{gcoo}{\&}}}'', object variables can be manipulated directly with Tcl commands, and calling the object variable directly as a command with no arguments (e.g. \texttt{[\$varObj]}) returns the object variable value.
